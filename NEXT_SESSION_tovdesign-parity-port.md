...
## ✅ 전 항목 완료 (2026-07-25 세션 종료)

**이 문서의 작업은 전부 끝났다.** 상세 결과·함정·남은 사용자 액션은
`tovhouse_20260725_d1전환-보안이식_세션핸드오프.md` 참조.

| 항목 | 상태 | 비고 |
| --- | --- | --- |
| §0 D1 마이그 | ✅ 완료 | 사용자 결정으로 Airtable·구글시트 폐지. D1 `tovhouse-db` |
| §1 토큰 위조 | ✅ 완료·배포 | 프로덕션 27/27 |
| §2 meta-webhook | ✅ 완료·배포 | 살아있는 호출자 0건 확인 후 강제 인증 |
| §3 internal-notify | ✅ 완료·배포 | 실제로 **무인증 통과 상태였음**(아래 참조) |
| §4 로그인 | ✅ 완료·배포 | 아이디/비번(scrypt) + 실패 카운터 D1. 새 인스턴스에서도 차단 유지 확인 |
| §5 접수폼 | ✅ 완료·배포 | CT/허니팟/타임스탬프 + rate limit 10회/시간(D1, 원자적, fail-open) |
| §6 그래프 | ✅ 완료·배포 | 실측: 730일 구코드 합계 0 → 신코드 167(=totals.users) |
| §7 Meta 폴링 | ✅ 완료·배포 | 매시 정각 cron, D1 저장 + 텔레그램. SMS는 기본 꺼둠 |
| §9 /api/track | ✅ 제거 | 인증 없이 CORS `*` 공개 쓰기 + 호출처 0건이었음 |
